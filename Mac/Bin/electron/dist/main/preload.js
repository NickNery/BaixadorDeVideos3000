...
electron_1.contextBridge.exposeInMainWorld("baixador", {
    ...
    launchDoom: () => electron_1.ipcRenderer.invoke("doom:launch"),
    ...
    onDoomEvent: (callback) => {
        const listener = (_event, payload) => callback(payload);
        electron_1.ipcRenderer.on("doom:event", listener);
        return () => electron_1.ipcRenderer.removeListener("doom:event", listener);
    }
});
