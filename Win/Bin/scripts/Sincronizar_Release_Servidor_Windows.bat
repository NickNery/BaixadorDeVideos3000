...
echo Preparando pastas Win e Mac...
powershell -NoProfile -ExecutionPolicy Bypass -File "%ROOT%\Bin\scripts\Preparar_Distribuicao_Plataformas.ps1"
if errorlevel 1 (
    echo [ERRO] Falha ao preparar a distribuicao Win/Mac.
    goto erro
)
...
robocopy "%ROOT%\%~1" "%DESTINO%\%~1" /MIR /R:2 /W:2 /NFL /NDL /NJH /NJS /XD build node_modules dist-packages .venv __pycache__ /XF package-lock.json BaixadorDeVideos3000_Setup_Windows.exe.tmp >nul
if %ERRORLEVEL% GEQ 8 (
...
powershell -NoProfile -ExecutionPolicy Bypass -Command ^
  ...
  "$oldDirs = @('Bin','setup','.venv','python','electron','docs','scripts','release','assets','build','src'); " ^
  "$oldFiles = @('.DS_Store','BaixadorDeVideos3000.vbs','BaixadorDeVideos3000.command','BaixadorDeVideos3000_Electron.exe','BaixadorDeVideos3000_Electron.command','BaixadorDeVideos3000_Electron.log','BaixadorDeVideos3000_Electron.log.err','BaixadorDeVideos3000_Electron.log.out','requirements.txt','ytdlp_gui_config.json'); " ^
