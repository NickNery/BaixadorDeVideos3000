...
contextBridge.exposeInMainWorld("baixador", {
  ...
  launchDoom: () => ipcRenderer.invoke("doom:launch"),
  ...
  onDoomEvent: (callback: (event: unknown) => void) => {
    const listener = (_event: Electron.IpcRendererEvent, payload: unknown) => callback(payload);
    ipcRenderer.on("doom:event", listener);
    return () => ipcRenderer.removeListener("doom:event", listener);
  }
});
